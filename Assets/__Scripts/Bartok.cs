...
public class Bartok : MonoBehaviour
{
    ...
    public bool ValidRankPlay (CardBartok cb, CardBartok target) {
        if (cb.rank == target.rank) return (true);
        return false;
    }
        ...
        switch (tCB.state) {
            ...
            case CBState.hand:
                if (ValidPlay(tCB)) {
                    ...
                    if (tCB.rank == 2) {
                        Debug.Log("human plays 2");
                    }

                    List<CardBartok> validCards = new List<CardBartok>();

                    foreach (CardBartok hCB in CURRENT_PLAYER.hand) {
                        if (ValidRankPlay(hCB, tCB)) {
                            validCards.Add(hCB);
                        }
                    }
                    MoveToTarget(tCB);
                    ...
                    if (validCards.Count > 0) {
                        foreach (CardBartok vCB in validCards) {
                            CURRENT_PLAYER.RemoveCard(vCB);
                            MoveToTarget(vCB);
                            lCB = vCB;
                        }
                    }

                    lCB.callbackPlayer = CURRENT_PLAYER;
