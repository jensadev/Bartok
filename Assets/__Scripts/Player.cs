...
public class Player
{
    ...
    public void TakeTurn () {
        ...
        validCards = new List<CardBartok>();

        foreach (CardBartok hCB in hand) {
            if (Bartok.S.ValidRankPlay(hCB, cb)) {
                validCards.Add(hCB);
            }
        }
        ...
        if (validCards.Count > 0) {
            foreach (CardBartok vCB in validCards) {
                RemoveCard(vCB);
                Bartok.S.MoveToTarget(vCB);
                lCB = vCB;
            }
        }
       lCB.callbackPlayer = this;
    }
